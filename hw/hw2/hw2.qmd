...
#| eval: false
result <- data.frame(
  read_types = c("Base R (read.csv)", "Tidyverse (read_csv)", "Data.table (fread)"),
...
mem_time_reingest <- system.time({
  df_reingest <- read_csv("~/mimic/hosp/admissions.csv.gz", col_types = col_types_defined)
})
...
```{bash}
#| eval: false
zcat < ~/mimic/hosp/labevents.csv.gz | awk -F',' '{if ($5 == 50912 || $5 == 50971 || $5 == 50983 || $5 == 50902 || $5 == 50882 || $5 == 51221 || $5 == 51301 || $5 == 50931) print $2 "," $5 "," $7 "," $10}' | gzip > ~/203b/hw/hw2/labevents_filtered.csv.gz
...
system.time({
  labevents_parquet <- open_dataset("labevents_parquet", format = "parquet") %>% filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>% select(subject_id, itemid, charttime, valuenum) %>%
    collect()
})

nrow(labevents_parquet)
head(labevents_parquet, 10)
```
...
execution_time <- system.time({
  labevents_arrow <- open_dataset("labevents_parquet", format = "parquet") %>% collect() 
  labevents_duckdb <- copy_to(con, labevents_arrow, "labevents", temporary = TRUE)
...
awk -F',' '{if ($7 == 220045 || $7 == 220181 || $7 == 220179 || $7 == 223761 || 
$7 == 220210) print $7}' | \
